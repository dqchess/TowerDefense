...
public class IceDemon : MonoBehaviour
{
    ...
    public GameObject iceDemonChild;

    private const float HEALTH = 350f;
    private const float RANGE_ATTACK = 1.2f;
    private const float ANGLE_SWAP_STATE = 60f;
    private const int EXP_RECEIVE_IF_ICEDEMON_DIE = 15;
    private const int GOLD_RECEIVE_IF_ICEDEMON_DIE = 30;
    ...
    private void Die()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            GameObject child = (GameObject)Instantiate(iceDemonChild, new Vector3(transform.position.x +
                                       Random.Range(-0.3f, 0.5f), transform.position.y +
                                       Random.Range(-0.3f, 0.5f), 0f), transform.rotation);
            child.GetComponentInChildren<IceDemonChild>().SetPointIceDemonParentDie(countPoint);
        }
        Destroy(gameObject);
    }
